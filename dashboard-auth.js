...
async function gate(opts){
  ...
  // Bajar TODO el data.json para saber si hay PIN (y para detectar inconsistencias)
  let fullData;
  try {
    fullData = await window.GitHubSync.fetchFullData();
  } catch(err){
  ...
  const security = fullData.__security || {};
  const hasPin = !!security[section];

  if(!hasPin){
    // ── PROTECCIÓN CRÍTICA ──
    // Si NO hay PIN para esta sección, hay dos casos:
    //   A) primera vez → ok, crear PIN
    //   B) algo se rompió → NO crear PIN (sobrescribiría datos)
    //
    // Heurística para distinguir: si hay datos de OTROS dashboards
    // pero NO hay PIN para ninguno, es sospechoso (probablemente
    // el data.json está roto). Bloqueamos y avisamos.
    const KNOWN_SECTIONS = ['training','options','patrimonio'];
    const otherSectionsWithData = KNOWN_SECTIONS.filter(s =>
      s !== section && fullData[s] && Object.keys(fullData[s]).length > 0
    );
    const totalPins = Object.keys(security).length;

    // Caso sospechoso: hay datos de otros dashboards pero ningún PIN configurado
    if(otherSectionsWithData.length > 0 && totalPins === 0){
      return showRecoveryWarning(ui, section, otherSectionsWithData);
    }
    // Caso normal: primera vez configurando PIN
...
// Pantalla de aviso cuando detectamos que algo no encaja
function showRecoveryWarning(ui, section, otherSections){
  return new Promise((resolve) => {
    ui.querySelector('#dagTitle').textContent = '⚠ Estado inesperado';
    ui.querySelector('#dagInfo').innerHTML = `
      <div style="text-align:left;color:#fbbf24;line-height:1.6">
        Hay datos de otros dashboards (<b>${otherSections.join(', ')}</b>) pero no encuentro tus PINs.<br><br>
        Esto puede significar que el archivo de datos remoto está incompleto.
        <b>NO crees un PIN nuevo</b>: podrías sobrescribir tus datos.<br><br>
        Lo más seguro es volver al inicio y revisar el data.json en GitHub
        (puede que necesites restaurar de un commit anterior).
      </div>`;
    ui.querySelector('#dagFields').innerHTML = '';
    ui.querySelector('#dagPrimary').style.display = 'none';
    wireBackButton(ui);
  });
}
