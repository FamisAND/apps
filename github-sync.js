...
async function pullRaw(){
  ...
  // El endpoint contents/ NO devuelve `content` para archivos >1 MB.
  // Para esos archivos hay que usar Git Data API (blobs) que sí los lee.
  if(data.content){
    try { content = JSON.parse(b64decode(data.content)); }
    catch(err){ const e = new Error('data.json corrupto'); e.cause = err; throw e; }
  } else if(data.sha){
    // Archivo grande: pedir el blob por su sha
    const blobRes = await ghFetch(`git/blobs/${data.sha}?_=${Date.now()}`,
                                  { cache: 'no-store' });
    if(!blobRes.ok){
      const e = new Error('Error leyendo blob de GitHub: '+blobRes.status);
      e.status = blobRes.status; throw e;
    }
    const blobData = await blobRes.json();
    if(blobData.content){
      try { content = JSON.parse(b64decode(blobData.content)); }
      catch(err){ const e = new Error('data.json corrupto (blob)'); e.cause = err; throw e; }
    }
  }
